...
final class Game_StartBonusTests: XCTestCase {
    ...
    func test_passEarth_collect500() {
        ...
        let cashBefore = game.state.players[0].federons
        
        let _ = game.roll(3, 1)
        
        XCTAssertEqual(game.state.players[0].federons, cashBefore+500)
    }
    func test_landOnStartDirectly_collect1000() {
        ...
        XCTAssertEqual(game.state.players[0].federons, cashBefore+1000)
    ...
    func test_blastOffFromEarth_doNotCollectAnyMoney() {
        ...
        XCTAssertEqual(game.state.players[0].federons, cashBefore)
    }
    func test_haveToStayOnEarthForMove_doNotCollectAnyMoney() {
        let cashBefore = game.state.players[0].federons
        game.state.players[0].hydrons = 3
